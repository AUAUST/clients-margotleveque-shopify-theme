document.addEventListener('DOMContentLoaded', function(){
  ...
  function nextSlide(){    
    ...
    if(!t.hasClass('transition') && !$('#type-row span').is(':focus')){
      ...
      t.parents('.slider').next().find('.counter span').text(t.find('.slide:first-child').data('slide'));
    }
  }
  function prevSlide(){
    var t = $('.slider-tracker.focus');
  ...
  function checkKey(e){
    e = e || window.event;
    if (e.keyCode == '37'){
      if($('body').hasClass('template-collection') && $(window).width() < 813){
        prevSlide()         
  ...
  $('.project').each(function(){
    $('header ul').append('<li class="link-bar" data-id="'+ $(this).data('id') +'">'+ $(this).data('title') +'</li>')
  });  
  $('header ul li').click(function(){
    var rem = $('#projects').css('padding-top'),
        px = parseInt(rem, 10);
    $('html, body').animate({
      scrollTop: $('.project[data-id="'+$(this).data('id')+'"]').offset().top - px
    }, 'medium', 'easeInOutQuint');
  });
    ...
    let o = new IntersectionObserver(function(E, observer){
      E.forEach(function(e){
        if (e.isIntersecting){
          ...
          $('header ul li[data-id="'+p.dataset.id+'"]').addClass('active');
        }
      });
  ...
  $('.style.product-selector').click(function(e){ 
    ...
    $('form#product-'+t.data('id')+' .options-wrapper ').removeClass('disabled');
    if(t.hasClass('style')){
